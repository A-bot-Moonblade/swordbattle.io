...
class EventToken extends Entity {
  ...
  constructor(game, objectData) {
    super(game, Types.Entity.EventToken, objectData);
    ...
    // Despawn event token after 2 minutes
  ...
  processTargetsCollision(player) {
    ...
    if (player.name === "Update Testing Account") {
      player.levels.addEventTokens(this.value * 50);
    } else {
      player.levels.addEventTokens(this.value);
    }
    
    player.flags.set(Types.Flags.GetEventToken, true);
